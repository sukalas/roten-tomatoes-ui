const DOM = (() => {
  ...
  const _templates = {
    articleContent: (data) => {
      ...
      return `
        ...
        <div class="movie__info">
          <h3 class="movie__info__title" title="${data.title}">${
        data.title
      }</h3>
    ...
    reviewsTemplate: ({ author, content }) => {
      const parsedTitle = Helpers.escapeSpecialCharacters(content);
      return `
    ...
    trailersTemplate: ({ name, key }) => {
      ...
      const parsedTitle = Helpers.escapeSpecialCharacters(name);
      return `
    ...
    similarMovieTemplate: ({ title, poster_path, vote_average }) => {
      const parsedTitle = Helpers.escapeSpecialCharacters(title);
      return `
  ...
  return {
    appendMovieList: appendMovieList,
